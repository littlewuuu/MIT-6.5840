...
func TestFollowerFailure3B(t *testing.T) {
	...
	leader2 := cfg.checkOneLeader()
	cfg.disconnect((leader2 + 1) % servers)
	LOG(servers, 0, DDrop, "disconnect S%d", (leader2+1)%servers)
	cfg.disconnect((leader2 + 2) % servers)
	LOG(servers, 0, DDrop, "disconnect S%d", (leader2+2)%servers)
...
func TestFigure8Unreliable3C(t *testing.T) {
	...
	for iters := 0; iters < 1000; iters++ {
		...
		if leader != -1 && (rand.Int()%1000) < int(RaftElectionTimeout/time.Millisecond)/2 {
			...
			LOG(servers, 0, DDrop, "disconnect S%d", leader)
			nup -= 1
		}
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	for i := 0; i < iters; i++ {
		...
		if disconnect {
			...
			LOG(3, 0, DDrop, "disconnect %d", victim)
			cfg.one(rand.Int(), servers-1, true)
		}
		if crash {
			cfg.crash1(victim)
			LOG(3, 0, DDrop, "crash %d", victim)
			cfg.one(rand.Int(), servers-1, true)
		}
		...
		if disconnect {
			...
			cfg.connect(victim)
			LOG(3, 0, DDrop, "re-connect %d", victim)
			cfg.one(rand.Int(), servers, true)
			leader1 = cfg.checkOneLeader()
		}
		if crash {
			...
			cfg.connect(victim)
			LOG(3, 0, DDrop, "restart %d", victim)
			cfg.one(rand.Int(), servers, true)
			leader1 = cfg.checkOneLeader()
		}
	}
